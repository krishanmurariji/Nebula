...
class NebulaAudioHandler extends BaseAudioHandler with SeekHandler {
  ...
  Future<void> _ensureCache() async {
    for (int i = 0; i < 50 && !_cacheReady; i++)
  ...
  Future<void> _load(Song song) async {
    ...
    try {
      ...
      if (_isCached(song.videoId)) {
        file = _cf(song.videoId);
        _markUsed(song.videoId);
        debugPrint('[N] cache hit ${file.lengthSync()}b');
      }

      if (file == null) {
        for (int attempt = 0; attempt < 4 && file == null; attempt++) {
          if (_tok != tok) return;
          try {
            final info = await _getInfo(song.videoId, tok);
            if (info == null) { debugPrint('[N] no info attempt $attempt'); break; }
            if (_tok != tok) return;
            file = await _download(info, song.videoId, tok);
            if (file == null && _tok != tok) return; // cancelled by user
          } on TimeoutException catch (e) {
            debugPrint('[N] attempt $attempt timeout: $e');
            _rotateYt();
            await Future.delayed(const Duration(milliseconds: 500));
          } catch (e, st) {
            debugPrint('[N] attempt $attempt error: $e\n$st');
            _rotateYt();
            await Future.delayed(const Duration(milliseconds: 500));
          }
        }
      ...
      if (file == null) {
        debugPrint('[N] ✗ all attempts failed for ${song.videoId}');
        playbackState.add(playbackState.value.copyWith(
            processingState: AudioProcessingState.error));
        return;
      }

      debugPrint('[N] setting source: ${file.path}');
      await _player.setAudioSource(AudioSource.file(file.path));
      if (_tok != tok) { _player.stop(); return; }
      _current = song;
      await _player.play();
      debugPrint('[N] ✓ PLAYING: ${song.title}');
      _prefetchNext(tok);
  ...
  Future<AudioStreamInfo?> _getInfo(String videoId, int tok) async {
    for (int ci = 0; ci < _clients.length; ci++) {
      ...
      try {
        ...
        debugPrint('[N] manifest c=$ci yt[${_pi%_pool.length}] $videoId');
        final m = await _yt.videos.streamsClient.getManifest(videoId, ytClients: [_clients[ci]])
            .timeout(const Duration(seconds: 15));
        if (m.audioOnly.isEmpty) { debugPrint('[N] c=$ci no audio'); continue; }
        ...
        final chosen = mp4.isNotEmpty
            ? mp4.firstWhere((s) => s.bitrate.bitsPerSecond >= 96000, orElse: () => mp4.last)
            : (m.audioOnly.toList()..sort((a, b) => a.bitrate.compareTo(b.bitrate))).first;
        debugPrint('[N] stream ${chosen.codec.mimeType} @ ${chosen.bitrate} c=$ci');
        return chosen;
      } on RequestLimitExceededException {
        debugPrint('[N] rate limit c=$ci'); _rotateYt();
      } on TimeoutException {
        debugPrint('[N] manifest timeout c=$ci');
      } catch (e) {
        debugPrint('[N] manifest error c=$ci: $e');
      }
    }
